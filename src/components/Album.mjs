        ...
        mode === "artwork"
          ? (
            ...
            <div class="gallery-info-card">
              <div class="info-card-inner" style={album.info.color && `background: ${album.info.color}`}>
                {album.info.title && (<h1 class="info-card-title">{album.info.title}</h1>)}
                <p class="info-card-description">{album.info.description}</p>
                {album.info.url && (
                  <div class="info-card-link"><a href={album.info.url}>Read more</a></div>
                )}
              </div>
            </div>
          )
